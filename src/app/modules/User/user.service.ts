...
const createAdmin = async(data: any) => {
    ...
    if (file) {
        ...
        data.body.admin.profilePhoto = uploadToCloudinary?.secure_url
    }

    const hashedPassword : string = await bcrypt.hash(data.body.password, 12)
    const userData = {
        email: data.body.admin.email,
...
const getAllFromDB = async (params: any, options: IPaginationOptions) => {
    const { page, limit, skip } = paginationHelper.calcultatePagination(options);
